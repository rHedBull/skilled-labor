...
- Wave Server running (default: `http://localhost:8000`)
...
```bash
curl -s -X POST http://localhost:8000/api/v1/projects/<project_id>/sequences \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X POST http://localhost:8000/api/v1/sequences/<sequence_id>/spec \
...
```bash
curl -s -X POST http://localhost:8000/api/v1/sequences/<sequence_id>/plan \
  -H 'Content-Type: text/plain' \
...
```bash
curl -s -X POST http://localhost:8000/api/v1/sequences/<sequence_id>/executions \
  -H 'Content-Type: application/json' \
  -d '{}' | jq .
```
...
```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id> | jq '{status, current_wave, completed_tasks, total_tasks}'
```

**Get task summary:**

```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id>/tasks | jq .
```
...
```bash
curl -s "http://localhost:8000/api/v1/executions/<execution_id>/events?limit=20" | jq '.[] | {event_type, task_id, phase, created_at}'
```

To get only new events since a timestamp:

```bash
curl -s "http://localhost:8000/api/v1/executions/<execution_id>/events?since=<ISO-timestamp>&limit=50" | jq .
```

**Get task output:**

```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id>/output/<task_id>
```

**Get task transcript:**

```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id>/transcript/<task_id>
```

**Get full execution log:**

```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id>/log
```
...
```bash
curl -s http://localhost:8000/api/v1/executions/<execution_id>/blockers | jq .
```

Resolve a blocker (retry the failed task or skip it):

```bash
curl -s -X POST http://localhost:8000/api/v1/executions/<execution_id>/blockers/<command_id> \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X POST http://localhost:8000/api/v1/executions/<execution_id>/cancel
```

**Continue from failure** (creates a new execution that resumes where the previous one stopped):

```bash
curl -s -X POST http://localhost:8000/api/v1/executions/<execution_id>/continue | jq .
```
...
```bash
curl -s http://localhost:8000/api/v1/projects/<project_id>/sequences | jq '.[] | {id, name, status}'
```

**List executions for a sequence:**

```bash
curl -s http://localhost:8000/api/v1/sequences/<sequence_id>/executions | jq '.[] | {id, status, trigger, completed_tasks, total_tasks}'
```

**Get spec or plan back:**

```bash
curl -s http://localhost:8000/api/v1/sequences/<sequence_id>/spec
curl -s http://localhost:8000/api/v1/sequences/<sequence_id>/plan
```

**Update a project:**

```bash
curl -s -X PATCH http://localhost:8000/api/v1/projects/<project_id> \
  -H 'Content-Type: application/json' \
  -d '{"name": "new name"}' | jq .
```
...
```bash
curl -s -X PATCH http://localhost:8000/api/v1/sequences/<sequence_id> \
  -H 'Content-Type: application/json' \
  -d '{"status": "planned"}' | jq .
```
...
```bash
curl -s -X POST http://localhost:8000/api/v1/projects/<project_id>/regenerate-key | jq .
```
